...
let historicoIndicadorAtual = 'matriculas';
...
function historicoListaUnidades(){
  ...
  // "INSTITUCIONAL" representa a linha Total da planilha de origem — trata como agregado, junto da Reitoria
  return [{ sigla: 'INSTITUCIONAL', nome: 'Total (Institucional)' }, ...uos]
    .filter(u => INDICADORES_HISTORICO_DATA.indicadores[historicoIndicadorAtual].valores[u.sigla]);
}
  ...
  return `
    <div class="filter-bar" style="align-items:flex-end;">
      <div class="filter-field" style="min-width:260px;">
        ...
        <select id="fHistoricoIndicador" onchange="onHistoricoIndicadorChange()">${opcoesIndicador}</select>
      </div>
...
function onHistoricoIndicadorChange(){
  historicoIndicadorAtual = document.getElementById('fHistoricoIndicador').value;
  historicoUnidadesFiltro = []; // unidades disponíveis podem mudar entre indicadores
...
function renderHistoricoTabela(el){
  const ind = INDICADORES_HISTORICO_DATA.indicadores[historicoIndicadorAtual];
...
function renderHistoricoGrafico(el){
  ...
  if(historicoUnidadesFiltro.length){
    ...
    // sem filtro: evita poluir o gráfico com todas as unidades — mostra só o Institucional por padrão
  ...
  historicoChartInstance = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        y: { beginAtZero: true, ticks: { callback: v => ind.formato === 'percentual' ? v + '%' : v } },
      }
    }
  });
  ...
  if(!historicoUnidadesFiltro.length){
    ...
    aviso.style.cssText = 'margin-top:10px; font-size:12px; color:var(--ink-soft);';
    aviso.textContent = 'Mostrando só o total institucional por padrão — selecione unidades no filtro para comparar campi.';
